...
function showtest(){
    sFtv.push("0808 测试连续三天第一天");
    sFtv.push("0809 测试连续三天第二天");
    sFtv.push("0809 二天");
    sFtv.push("0810 测试连续三天第三天");
}
//农历节日 *表示放假日
var lFtv = new Array(
"0101*春节",
...
function getFestv(date,ld){
    ...
    var length = solarDays(y,m); //国历当月天数
    ...
    for(i in lFtv)
        if(lFtv[i].match(/^(\d{2})(.{2})([\s\*])(.+)$/)) {
            var ln=ld;
            if(ln.month==Number(RegExp.$1) && ln.day==Number(RegExp.$2)){
                resstr+= (resstr==""?"":"|")+RegExp.$4;
            }
        }
    ...
    for(i in wFtv)
        if(wFtv[i].match(/^(\d{2})(\d)(\d)([\s\*])(.+)$/))
            if(Number(RegExp.$1)==(m+1)) {
                ...
                if(d == ((firstWeek>tmp2)?7:0) + 7*(tmp1-1) + tmp2 - firstWeek + 1)
                    resstr += (resstr==""?"":"|")+RegExp.$5;
            }
